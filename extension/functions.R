...
update_m_deltas <- function(Omegas, m1_alphas, m_tau, v0, v1, Tn, c=1) {
  
  lapply(1:Tn, function(t){
    1 / (1 + exp(
      c * log(v1 / v0) +
        c * m_tau * Omegas[[t]] ^ 2 * (1 / v1 ^ 2 - 1 / v0 ^ 2) / 2 +
...
get_dstars <- function(m_deltas, v0, v1) {
  
  lapply(m_deltas, function(x) x / v1 ^ 2 + (1-x) / v0 ^ 2)
...
update_alpha_tau <- function(a_tau, P, Tn, c = 1) {
  
  c * (Tn * P * (P - 1) / 4 + a_tau  - 1) + 1
  
}

update_beta_tau <- function(Omegas, dstars, b_tau, Tn, c = 1) {
...
e_omegas <- function(Omegas,
  ...
  sum(sapply(1:Tn, function(t){
    - lambda * sum(diag(Omegas[[t]])) / 2 -
      ...
      log(v0) * sum(1-m_deltas[[t]][bool_up]) -
      m_tau * sum(Omegas[[t]][bool_up]^2 * dstars[[t]][bool_up])/2 +
      m_log_tau * P * (P-1) / 4
  }))
}
