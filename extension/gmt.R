...
gmt <- function(Ys,
  ...
  if (!'Omegas' %in% names(list_init) | ('Omegas' %in% names(list_init) & is.null(list_init$Omegas))) {
    ...
    Sigmas <- lapply(1:Tn, function(t)(Ss[[t]] + v0 * diag(P))/Ns[t])
  ...
  while ((ELBO_CM_diff > tol & (it < maxit))) {
    ...
    while (ELBO_diff > tol & VB_it < maxit) {
      ...
      # % # m_deltas
      m_deltas <- update_m_deltas(Omegas, m1_alphas, m_tau, v0, v1, Tn) 
      dstars <- get_dstars(m_deltas, v0, v1)
      # % #
    ...
    #   }
    # } else {
    Omegas <- get_omegas(lapply(dstars,function(x) x * m_tau), 
