...
GMx_gen <- function(n,p,list_hyper_x, X = c(1,2,3,4)){
  ...
  for (k in 1:length(X)){
    ...
    for (i in 1:p){
      for (j in 1:i){
        RHO[k,i,j] <- pnorm(zeta + a*BETA[i,j])
        if (i!=j){
          RHO[k,j,i] <- RHO[k,i,j]  
        }
      }
  ...
  for (k in 1:length(X)){
    ...
    for (i in 1:p){
      OMEGA[k,i,i] <- rexp(1,lambda/2)
            for (j in 1:i-1){
              if (DELTA[i,j] == 1){
                OMEGA[k,i,j] <- rnorm(1,0,var1)
                OMEGA[k,j,i] <- omega[k][i,j]
              } else {
                OMEGA[k,i,j] <- rnorm(1,0,var0)
                OMEGA[k,j,i] <- omega[k,i,j]
              }
            }
    }
  ...
  for (omega in OMEGA){
    ...
    omega_bar <- (omega + t(omega)) / 2
    
    min_eigen <- min(eigen(omega_bar, only.values = TRUE)$values)
    if (min_eigen < 0) {
      omega <- omega_bar + (0.1 - min_eigen) * diag(p)
    } else{
      omega <- omega_bar + 0.1 * diag(p)
    }
