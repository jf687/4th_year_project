...
GM_ROC_sim <- function(ggm.sf,y, list_hyper, list_init, v0.vals, t.vals){
  ...
  Omega.true[which(abs(Omega.true)<1e-5)]=0 
...
GM_average <- function(ggm.sf, N, list_hyper, list_init, v0.vals, t.vals, plot = FALSE){
...
plot_ssl_ROC <- function(res.ssl, v0){
  v0 <- round(v0,3)
  auc_value = res.ssl$auc_value
  perf = res.ssl$perf
  plot(perf, main = '', xlim = c(0, 1), ylim = c(0, 1), lwd = 2, colorize = FALSE)
